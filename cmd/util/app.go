...
func AddAppFlags(command *cobra.Command, opts *AppOptions) {
	...
	command.Flags().StringVar(&opts.retryBackoffDuration, "retry-backoff-duration", fmt.Sprintf("%ds", common.DefaultSyncRetryDuration/time.Second), "Retry backoff base duration. Default unit is seconds, but could also be a duration (e.g. 2m, 1h)")
	command.Flags().StringVar(&opts.retryBackoffMaxDuration, "retry-backoff-max-duration", fmt.Sprintf("%ds", common.DefaultSyncRetryMaxDuration/time.Second), "Max retry backoff duration. Default unit is seconds, but could also be a duration (e.g. 2m, 1h)")
			...
			if appOpts.retryLimit > 0 {
				...
				spec.SyncPolicy.Retry = &argoappv1.RetryStrategy{
					...
					Backoff: &argoappv1.Backoff{
						Duration:    appOpts.retryBackoffDuration,
						MaxDuration: appOpts.retryBackoffMaxDuration,
